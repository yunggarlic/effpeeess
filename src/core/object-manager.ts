...
export class ObjectManager {
    ...
    addMany(objects: GameObject[]) {
        const { collidables, bullets, uncategorized, players, } = objects.reduce((acc, object) => {
            switch (object.type) {
                case GameObjectTypes.Player:
                case GameObjectTypes.Collidable:
        ...
        }, { players: [] as number[], collidables: [] as number[], bullets: [] as number[], uncategorized: [] as number[] });
    ...
    remove(object: GameObject) {
        switch (object.type) {
            case GameObjectTypes.Player:
                ...
                this.collidables = this.collidables.filter((id) => id !== object.mesh.id);
                break;
            case GameObjectTypes.Collidable:
                this.collidables = this.collidables.filter((id) => id !== object.mesh.id);
                break;
            case GameObjectTypes.Bullet:
                ...
                break;
            default:
                this.uncategorized = this.uncategorized.filter((id) => id !== object.mesh.id);
