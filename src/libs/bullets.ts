...
export function updateBullets(delta: number) {
    for (let i = gameState.objectManager.bullets.length - 1; i >= 0; i--) {
        const bullet = gameState.objectManager.bullets[i];
        ...
        for (const collidable of gameState.objectManager.collidables) {
            const collidableBox = new THREE.Box3().setFromObject(collidable.mesh);
...
export class Bullet extends Collidable {
    ...
    constructor({ geometry, material }: BulletProps, velocity: THREE.Vector3, originator: THREE.Vector3) {
        super({ geometry, material, isCollidable: true, type: GameObjectTypes.Bullet });
